...
def test_property_23_rejection_only_for_pending(dynamodb_table, admin_event_factory, context, initial_status, should_succeed):
    """
    Property 23: Admin rejection should only work on pending requests
    
    For any rental request, admin rejection should succeed if and only if
    the request has status "pending".
...
def test_property_24_rejection_reason_optional(dynamodb_table, admin_event_factory, context, include_reason):
    ...
    if include_reason:
        ...
        assert 'rejection_reason' not in body['data']
...
def test_rejection_preserves_request_data(dynamodb_table, admin_event_factory, context):
    ...
    assert body['data']['boat_type'] == original_data['boat_type']
    assert body['data']['requester_id'] == original_data['requester_id']
    assert body['data']['requester_email'] == original_data['requester_email']
    assert body['data']['created_at'] == original_data['created_at']
    
    # Verify rejection fields are added
    assert body['data']['status'] == 'rejected'
    assert body['data']['rejected_by'] == admin_id
    assert 'rejected_at' in body['data']
