...
/**
 * Rental Request API Methods
 * New request-based rental system
 */

/**
 * Create a new rental request
 * @param {Object} requestData - Request data
 * @param {string} requestData.boat_type - Type of boat (skiff, 4-, 4+, 4x-, 4x+, 8+, 8x+)
 * @param {string} requestData.desired_weight_range - Desired weight range (e.g., "70-90kg")
 * @param {string} requestData.request_comment - Comment/notes for the request
 * @returns {Promise} API response
 */
export async function createRentalRequest(requestData) {
  return apiClient.post('/rental/request', requestData);
}

/**
 * Get team manager's own rental requests
 * @returns {Promise} API response with rental_requests array
 */
export async function getMyRentalRequests() {
  return apiClient.get('/rental/my-requests');
}

/**
 * Get accepted rental requests ready for payment
 * @returns {Promise} API response with rental_requests array including pricing
 */
export async function getRentalsForPayment() {
  return apiClient.get('/rental/requests-for-payment');
}

/**
 * Cancel a rental request
 * @param {string} rentalRequestId - ID of the rental request to cancel
 * @returns {Promise} API response
 */
export async function cancelRentalRequest(rentalRequestId) {
  return apiClient.delete(`/rental/request/${encodeURIComponent(rentalRequestId)}`);
}

/**
 * Admin Rental Request API Methods
 * Admin interface for managing rental requests
 */

/**
 * Get all rental requests (admin only)
 * @param {Object} filters - Optional filters
 * @param {string} filters.status - Filter by status (pending, accepted, paid, cancelled)
 * @param {string} filters.boat_type - Filter by boat type
 * @returns {Promise} API response with rental_requests array
 */
export async function listRentalRequests(filters = {}) {
  const params = new URLSearchParams();
  if (filters.status) params.append('status', filters.status);
  if (filters.boat_type) params.append('boat_type', filters.boat_type);
  
  const queryString = params.toString();
  const url = queryString ? `/admin/rental-requests?${queryString}` : '/admin/rental-requests';
  
  return apiClient.get(url);
}

/**
 * Accept a rental request and provide assignment details (admin only)
 * @param {string} rentalRequestId - ID of the rental request
 * @param {string} assignmentDetails - Assignment details (boat number, oars, location, etc.)
 * @returns {Promise} API response
 */
export async function acceptRentalRequest(rentalRequestId, assignmentDetails) {
  return apiClient.put(
    `/admin/rental-requests/${encodeURIComponent(rentalRequestId)}/accept`,
    { assignment_details: assignmentDetails }
  );
}

/**
 * Update assignment details for an accepted request (admin only)
 * @param {string} rentalRequestId - ID of the rental request
 * @param {string} assignmentDetails - Updated assignment details
 * @returns {Promise} API response
 */
export async function updateAssignmentDetails(rentalRequestId, assignmentDetails) {
  return apiClient.put(
    `/admin/rental-requests/${encodeURIComponent(rentalRequestId)}/assignment`,
    { assignment_details: assignmentDetails }
  );
}
 ...
 * @returns {Promise} API response
 */
export async function rejectRentalRequest(rentalRequestId, rejectionReason = null) {
  const payload = rejectionReason ? { rejection_reason: rejectionReason } : {};
  return apiClient.delete(
    `/admin/rental-requests/${encodeURIComponent(rentalRequestId)}`,
    { data: payload }
  );
}

/**
 * Reset an accepted rental request back to pending (admin only)
 * @param {string} rentalRequestId - ID of the rental request
 * @returns {Promise} API response
 */
export async function resetRentalRequest(rentalRequestId) {
  return apiClient.put(
    `/admin/rental-requests/${encodeURIComponent(rentalRequestId)}/reset`
  );
}
